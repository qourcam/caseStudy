...
public class OrderController {
    ...
    public Page<BookOrder> getAllOrdersOfCustomer(@RequestParam(defaultValue = "0") Integer pageNo,
                                                  @RequestParam(defaultValue = "10") Integer pageSize,
                                                  @RequestParam(defaultValue = "id") String sortBy,
                                                  @ApiParam(required = true) @PathVariable("id") UUID customerId) {

        Pageable pageable = PageRequest.of(pageNo, pageSize, Sort.by(sortBy));
        return orderService.findOrdersOfCustomer(customerId, pageable);
    }
